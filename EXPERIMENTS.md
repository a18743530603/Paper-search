...
- 真实缓存预热：已完成，运行 `#7`。
- 缓存复跑耗时验证：已完成，运行 `#8`。
- Git 功能提交：已完成，`1f200c9 Cache repeatable RAG experiments`。
- Git 文档提交：已完成，`637b939 Record v0.7 cache verification`。
- Git 推送：已完成，`main` 与 `v0.7.0` 已上传到 GitHub。
- Web 页面验收：已完成，`GET /evaluation/runs/8` 返回 HTTP 200，页面显示问题向量缓存和 `4.41 秒`总耗时。
...
```text
http://127.0.0.1:8003/evaluation/runs/8
```

验收结果：HTTP 200；缓存统计区域可见；运行总耗时显示为 `4.41 秒`。本地服务使用隐藏进程启动，当前页面已加载新版本代码。

### 尚未得出的结论

- 缓存不改变 E0/E1 的效果结论，也不能提升 Hit@K；它只降低重复实验时间和 API 调用次数。
- 当前只验证了同配置复跑约快 `24.0` 倍；更换分块策略后的实际复用率和耗时尚未实测。
- C0 已完成；章节前缀、中英查询、RRF、重排、父子块和表格分块实验均未开始。
