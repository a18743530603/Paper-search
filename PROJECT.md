...
当前已完成 DeepSeek 真实模型调用、Seed1.6 `Doubao-embedding-vision-251215` 稠密语义向量、本地 TF-IDF 关键词向量、双路混合召回、后台问答、页码证据展示和固定分块基线评测。下一阶段可对比结构化分块、语义分块与重排序。
...
- 唯一未命中题的完整证据块位于第 18 名，后续需要分别验证分块策略、跨语言查询和重排序的影响。
