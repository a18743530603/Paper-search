...
- `benchmarks/chunking_baseline.csv` 保存 3 篇论文的 15 条人工标注问题、参考答案、物理页码和原文证据。
...
- 当前 `length_boundary` 基线参数为 `1200/150`，实测 `Hit@5=93.33%`、`MRR@5=0.7667`。
- 评测页可配置块大小、重叠、Top-K 和 Seed/TF-IDF 权重；每次运行都会强制重建活动分块与索引，并将参数和结果保存为独立运行。
- `900/120` 对照实验产生 `217` 个文本块，`Hit@1` 从 `66.67%` 降到 `53.33%`、MRR 从 `0.7667` 降到 `0.6856`，证明更细分块不一定改善排序。
- 新增 `academic_section`：识别数字/罗马数字/IEEE 字母章节和附录层级，跨页继承章节路径，在章节内生成定长子块，并过滤参考文献与致谢。
- 章节修正版实验生成 `148` 块、识别 `44` 条章节路径，实测 `Hit@1=60.00%`、`Hit@5=80.00%`、`MRR@5=0.6889`、覆盖率 `83.30%`；它改善了结构与噪声控制，但没有超过固定边界基线。
- 两道完整证据位于第 6、7 名，后续需要分别验证中英查询扩展、章节标题独立向量和重排序的影响。
