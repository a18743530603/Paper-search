...
5. 等待页面显示“全部已建立索引”，再点击“运行基线评测”。
6. 查看总体指标、Top-K 曲线和逐题结果；展开“查看 Top 5 召回片段”可以检查具体召回内容。
...
每次点击“运行基线评测”都会创建新的 `evaluation_runs` 记录。当前运行曲线展示 Hit@1、Hit@3、Hit@5 的变化；历史趋势图按运行编号比较多次结果。当前 `v0.4.1` 页面仍使用固定基线参数，重复运行通常会得到相同结果；历史趋势图先用于保存和观察运行记录，后续加入可配置分块策略并强制重建索引后，才用于正式的策略对照实验。
...
负责固定分块基线评测：按论文标题导入本地 PDF 与人工标注 CSV，使用“同页 + 规范化 token 覆盖率”识别正确证据，并计算 `Hit@1/3/5`、`Recall@1/3/5`、`MRR@5` 和平均证据覆盖率。评测只调用检索器，不调用 DeepSeek 生成答案。
